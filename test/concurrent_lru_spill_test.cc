...
void ConcurrentPutWithClient(std::string ipc_socket) {
    ...
    auto create_and_seal_array = [&](Client& c) {
        ...
        auto sealed_array = std::dynamic_pointer_cast<Array<double>>(builder.Seal(c));
    };
...
void ConcurrentPutWithRPCClient(std::string rpc_endpoint) {
    ...
    auto create_remote_blob = [&](RPCClient& c) {
        auto remote_blob_writer = std::make_shared<RemoteBlobWriter>(double_array.size() * sizeof(double));
        std::memcpy(remote_blob_writer->data(), double_array.data(), double_array.size() * sizeof(double));
        ObjectMeta meta;
...
void ConcurrentGetAndPut(std::string ipc_socket, std::string rpc_endpoint) {
    ...
    auto create_and_seal_array = [&](Client& c) {
        auto double_array = InitArray<double>(array_size, [](int i) { return static_cast<double>(i); });
        ArrayBuilder<double> builder(c, double_array);
        auto sealed_array = std::dynamic_pointer_cast<Array<double>>(builder.Seal(c));
        return GetObjectID(sealed_array);
    };
