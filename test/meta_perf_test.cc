...
int main(int argc, char** argv) {
  if (argc < 2) {
    printf("usage ./meta_perf_test <ipc_socket>");
    return 1;
  }
  ...
  for (size_t i = 0; i < 1000; ++i) {
    LOG_EVERY_N(INFO, 10) << "perf simple: round " << i;
    perfSimple(client);
  }

  LOG(INFO) << "Passed metadata perf tests...";
