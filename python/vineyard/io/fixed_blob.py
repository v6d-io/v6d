...
class FixedBlobStream(BaseStream):
    ...
    def open(self, client, mode, wait: bool = False, timeout: int = 0):
        self.client_ = client
        if self.is_remote_:
            self.recv_mem_fd_ = (
                self.client_.vineyard_open_remote_fixed_stream_with_name(
                    self.stream_name_,
                    self.meta.id,
                    self.nums_,
                    self.size_,
        ...
        try:
            self.recv_mem_ = mmap.mmap(
                self.recv_mem_fd_, self.mmap_size, access=mmap.ACCESS_READ
            )
    ...
    def activate_stream_with_offset(self, offsets: List[int]):
        ...
        self.client_.vineyard_activate_remote_fixed_stream_with_offset(
            self.meta.id, offsets
        )
...
def register_fixed_blob_stream_types(_builder_ctx, resolver_ctx):
    if resolver_ctx is not None:
        resolver_ctx.register('vineyard::FixedBlobStream', fixed_blob_stream_resolver)
