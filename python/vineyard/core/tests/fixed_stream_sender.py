...
def check_received(client:vineyard.Client, stream_id: vineyard.ObjectID, stream_reader: FixedBlobStream.Reader):
  finished = False
  while not finished:
...
def run_sender(client: vineyard.Client, mm: mmap.mmap):
  fixed_blob_stream = FixedBlobStream.new(client, "test-stream-5", blob_num, blob_size, False, "")
  ...
  thread = threading.Thread(target=check_received, args=(client, id, stream_writer,))
