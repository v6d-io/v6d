...
void EtcdMetaService::TryAcquireLock(std::string key,
  ...
  etcd_->lock(prefix_ + key).then([self, callback_after_try_lock](
    ...
    if (resp.is_ok()) {
      ...
      self->server_ptr_->GetMetaContext().post(
          boost::bind(callback_after_try_lock, Status::OK(), true, resp.lock_key().substr(self->prefix_.size())));
    } else {
...
void EtcdMetaService::TryReleaseLock(
  ...
  etcd_->unlock(prefix_ + key).then([self, callback_after_try_unlock](
                                 pplx::task<etcd::Response> const& resp_task) {
    auto const& resp = resp_task.get();
    if (resp.is_ok()) {
