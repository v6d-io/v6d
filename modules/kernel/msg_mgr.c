...
static int send_msg(void)
{
	...
	memcpy(&msg.msg.request, &global_msg,
	       sizeof(struct vineyard_request_msg));
...
static int map_vineyard_user_storage_buffer(unsigned long user_addr)
{
	...
	vineyard_storage_kernel_addr =
		vmap(pages, page_num, VM_MAP, PAGE_KERNEL);
...
static unsigned long prepare_user_buffer(void **kernel_addr, size_t size)
{
	...
	*kernel_addr = kzalloc(size, GFP_KERNEL);
	...
	user_addr =
		vm_mmap(NULL, 0, size, PROT_READ | PROT_WRITE, MAP_SHARED, 0);
	vma = find_vma(current->mm, user_addr);
	remap_pfn_range(vma, user_addr, PFN_DOWN(__pa(*kernel_addr)), size,
			vma->vm_page_prot);
...
{
	vineyard_object_info_user_addr = (void *)prepare_user_buffer(
		&vineyard_object_info_kernel_addr, PAGE_SIZE);
	vineyard_object_info_header = vineyard_object_info_kernel_addr;
	vineyard_object_info_buffer_addr =
		vineyard_object_info_kernel_addr +
		sizeof(struct vineyard_object_info_header);
	global_msg.param._set_param.obj_info_mem =
		(uint64_t)vineyard_object_info_user_addr;
}
...
static void handle_set_bulk(struct vineyard_result_msg *rmsg)
{
	...
	printk(KERN_INFO PREFIX "%llx %llx %d\n", bulk_addr, bulk_size,
	       rmsg->ret._set_ret.ret);

	page_num = (PAGE_UP(bulk_addr + bulk_size) - PAGE_DOWN(bulk_addr)) /
		   PAGE_SIZE;
	printk(KERN_INFO PREFIX "page_num:%lu size:%lu\n", page_num,
	       page_num * 0x1000);
	err = map_vineyard_user_storage_buffer(PAGE_DOWN(bulk_addr));

	vineyard_bulk_kernel_addr =
		bulk_addr - PAGE_DOWN(bulk_addr) + vineyard_storage_kernel_addr;
...
{
	nl_socket = (struct sock *)netlink_kernel_create(
		&init_net, NETLINK_VINEYARD, &cfg);
	if (!nl_socket) {
