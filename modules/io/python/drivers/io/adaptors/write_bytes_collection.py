...
from vineyard.io.utils import BaseStreamExecutor, ThreadStreamExecutor, expand_full_path, report_error, report_exception
...
def write_metadata(streams: StreamCollection, prefix: str, storage_options: Dict):
    ...
    for k, v in streams.meta.items():
        metadata[k] = v
    metadata_path = os.path.join(prefix, metadata[StreamCollection.KEY_OF_PATH], 'metadata.json')
...
def write_stream_collections(client, stream_id: ObjectID, blob_queue: "ConcurrentQueue[ObjectID]", worker_prefix: str,
    ...
    if isinstance(streams, StreamCollection):
        ...
        for stream in streams.streams:
            write_stream_collections(client, stream, blob_queue, worker_prefix, storage_options)
    else:
...
def main():
    if len(sys.argv) < 7:
        print("usage: ./write_bytes_collection <ipc_socket> <prefix> <stream_id> <storage_options> "
              "<proc_num> <proc_index>")
        exit(1)
    ...
    write_bytes_collection(ipc_socket, prefix, stream_id, storage_options, proc_num, proc_index)
