...
from vineyard.io.utils import BaseStreamExecutor, ThreadStreamExecutor, expand_full_path, report_error, report_exception, report_success
...
def read_byte_stream(client, fs: AbstractFileSystem, stream: ByteStream, path: str, chunk_size: int = CHUNK_SIZE):
...
class ReadToByteStreamExecutor(BaseStreamExecutor):
    ...
    def execute(self):
        ...
        while True:
            ...
            total_size = read_byte_stream(self._client, self._fs, stream, path, self._chunk_size)
            processed_blobs += 1
    ...
    if fs.exists(metadata_path):
        ...
        for path in fs.listdir(prefix):
            if path['type'] == 'directory':
                streams.append(read_stream_collections(client, fs, queue, base_prefix, path['name']))
        ...
        with fs.open(blob_path, 'rb') as f:
            ...
            stream = ByteStream.new(client,
                                    params={
                                        StreamCollection.KEY_OF_PATH: os.path.relpath(blob_path, base_prefix),
                                        'length': total_size,
                                    })
...
def read_bytes_collection(vineyard_socket, prefix, storage_options, proc_num, proc_index):
    ...
    executor = ThreadStreamExecutor(ReadToByteStreamExecutor,
                                    parallism=1,
                                    client=client,
                                    fs=fs,
                                    task_queue=queue,
...
def main():
    if len(sys.argv) < 6:
        print("usage: ./read_bytes_collection <ipc_socket> <prefix> <storage_options> <proc_num> <proc_index>")
        exit(1)
