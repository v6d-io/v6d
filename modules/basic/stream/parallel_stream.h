...
template <typename T>
class ParallelStream : public Registered<ParallelStream<T>> {
 public:
  static std::shared_ptr<Object> Create() __attribute__((used)) {
    return std::static_pointer_cast<Object>(
        std::make_shared<ParallelStream<T>>());
  }

  void Construct(const ObjectMeta& meta) override {
    std::string __type_name = type_name<ParallelStream<T>>();
    ...
    for (size_t idx = 0; idx < this->size_; ++idx) {
      streams_.emplace_back(std::dynamic_pointer_cast<T>(
          meta.GetMember("stream_" + std::to_string(idx))));
    }
  }

  std::shared_ptr<T> GetStream(int index) { return streams_[index]; }
 ...
 private:
  size_t size_;
  std::vector<std::shared_ptr<T>> streams_;
...
class ParallelStreamBuilder : public ObjectBuilder {
 public:
  ...
  std::shared_ptr<Object> _Seal(Client& client) override {
    ...
    auto __value = std::make_shared<ParallelStream<T>>();

    __value->meta_.SetTypeName(type_name<ParallelStream<T>>());
