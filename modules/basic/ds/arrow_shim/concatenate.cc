 ...
 *
 * https://github.com/apache/arrow/blob/master/cpp/src/arrow/array/concatenate.cc
 *
...
// make sure `ARROW_VERSION` defined
...
struct Bitmap {
  ...
  explicit Bitmap(const std::shared_ptr<Buffer>& buffer, Range r)
      : buffer(buffer), data(buffer ? buffer->data() : nullptr), range(r) {}
...
Status ConcatenateBitmaps(std::vector<Bitmap>&& bitmaps, MemoryPool* pool,
  ...
  for (auto bitmap : bitmaps) {
    if (bitmap.AllSet()) {
      ...
      internal::CopyBitmap(bitmap.data, bitmap.range.offset,
                           bitmap.range.length, dst, bitmap_offset);
    }
    bitmap_offset += bitmap.range.length;
    bitmap.buffer.reset();  // release the reference
  }
...
Status PutOffsets(const std::shared_ptr<Buffer>& src, Offset first_offset,
...
template <typename Offset>
Status ConcatenateOffsets(BufferVector&& buffers, MemoryPool* pool,
                          std::shared_ptr<Buffer>* out,
  ...
  ARROW_ASSIGN_OR_RAISE(
      *out, AllocateBuffer((out_length + 1) * sizeof(Offset), pool));
  ...
  for (size_t i = 0; i < buffers.size(); ++i) {
    ...
    buffers[i].reset();  // release the reference
  }
...
class ConcatenateImpl {
 public:
  ConcatenateImpl(ArrayDataVector&& in, MemoryPool* pool)
    ...
    for (size_t i = 0; i < in_.size(); ++i) {
      ...
      out_->null_count =
          SafeSignedAdd(out_->null_count.load(), in[i]->null_count.load());
    }
  ...
  Status Visit(const FixedWidthType& fixed) {
    ...
    return ConcatenateBuffers(std::move(buffers), pool_)
  ...
  Status Visit(const BinaryType&) {
    ...
    RETURN_NOT_OK(ConcatenateOffsets<int32_t>(
        std::move(index_buffers), pool_, &out_->buffers[1], &value_ranges));
    ARROW_ASSIGN_OR_RAISE(auto value_buffers, Buffers(2, value_ranges));
    return ConcatenateBuffers(std::move(value_buffers), pool_)
  ...
  Status Visit(const LargeBinaryType&) {
    ...
    RETURN_NOT_OK(ConcatenateOffsets<int64_t>(
        std::move(index_buffers), pool_, &out_->buffers[1], &value_ranges));
    ARROW_ASSIGN_OR_RAISE(auto value_buffers, Buffers(2, value_ranges));
    return ConcatenateBuffers(std::move(value_buffers), pool_)
        .Value(&out_->buffers[2]);
  ...
  Status Visit(const FixedSizeListType& fixed_size_list) {
    ARROW_ASSIGN_OR_RAISE(auto child_data,
                          ChildData(0, fixed_size_list.list_size()));
    return ConcatenateImpl(std::move(child_data), pool_)
        .Concatenate(&out_->child_data[0]);
  }
  ...
  Result<BufferVector> UnifyDictionaries(const DictionaryType& d) {
    ...
    ARROW_ASSIGN_OR_RAISE(auto unifier,
                          DictionaryUnifier::Make(d.value_type()));
    ...
    RETURN_NOT_OK(
        unifier->GetResultWithIndexType(d.index_type(), &out_dictionary));
  ...
  Result<std::shared_ptr<Buffer>> ConcatenateDictionaryIndices(
    ...
    const auto index_width =
        internal::checked_cast<const FixedWidthType&>(index_type).bit_width() /
        8;
    int64_t out_length = 0;
    ...
    ARROW_ASSIGN_OR_RAISE(auto out,
                          AllocateBuffer(out_length * index_width, pool_));
    ...
    for (size_t i = 0; i < in_.size(); i++) {
      ...
      if (!data->buffers[0]) {
        RETURN_NOT_OK(internal::TransposeInts(
            ...
            /*dest_offset=*/0, /*length=*/data->length, transpose_map));
      } else {
        ...
        while (true) {
          ...
          if (run.set) {
            RETURN_NOT_OK(internal::TransposeInts(
                index_type, index_type, src,
                /*dest=*/out_data,
                /*src_offset=*/data->offset + position,
                /*dest_offset=*/position, run.length, transpose_map));
          } else {
  ...
  Status Visit(const DictionaryType& d) {
    auto fixed =
        internal::checked_cast<const FixedWidthType*>(d.index_type().get());
    ...
    if (dictionaries_same) {
      ...
      return ConcatenateBuffers(std::move(index_buffers), pool_)
          .Value(&out_->buffers[1]);
    } else {
  ...
  Status Visit(const UnionType& u) {
    ...
    RETURN_NOT_OK(ConcatenateBuffers(std::move(type_buffers), pool_)
                      .Value(&out_->buffers[1]));
  ...
  Status Visit(const RunEndEncodedType& type) {
    ...
    for (const auto& input : in_) {
      if (internal::AddWithOverflow(
              physical_length, ree_util::FindPhysicalLength(ArraySpan(*input)),
              &physical_length)) {
        return Status::Invalid("Length overflow when concatenating arrays");
      }
    }
    ...
    RETURN_NOT_OK(
        internal::checked_cast<RunEndEncodedBuilder&>(*builder).ReservePhysical(
            physical_length));
    for (const auto& input : in_) {
      RETURN_NOT_OK(
          builder->AppendArraySlice(ArraySpan(*input), 0, input->length));
    }
 ...
 private:
  // NOTE: Concatenate() can be called during IPC reads to append delta
  // dictionaries on non-validated input.  Therefore, the input-checking
  // SliceBufferSafe and ArrayData::SliceSafe are used below.
  ...
  Result<BufferVector> Buffers(size_t index) {
    ...
    for (auto& array_data : in_) {
      auto& buffer = array_data->buffers[index];
      if (buffer != nullptr) {
        ARROW_ASSIGN_OR_RAISE(
            auto sliced_buffer,
  ...
  Result<BufferVector> Buffers(size_t index, const std::vector<Range>& ranges) {
    ...
    buffers.reserve(in_.size());
    for (size_t i = 0; i < in_.size(); ++i) {
      auto& buffer = in_[i]->buffers[index];
      if (buffer != nullptr) {
        ARROW_ASSIGN_OR_RAISE(
            auto sliced_buffer,
  ...
  std::vector<Bitmap> Bitmaps(size_t index) {
    ...
    for (size_t i = 0; i < in_.size(); ++i) {
      ...
      in_[i]->buffers[index].reset();  // release the reference
    }
    return bitmaps;
  }
  ...
  // Gather the index-th child_data of each input into a vector.
  // Elements are sliced with that input's offset and length multiplied by
  // multiplier.
  Result<ArrayDataVector> ChildData(size_t index, size_t multiplier) {
    ArrayDataVector child_data(in_.size());
    for (size_t i = 0; i < in_.size(); ++i) {
      ARROW_ASSIGN_OR_RAISE(child_data[i], in_[i]->child_data[index]->SliceSafe(
                                               in_[i]->offset * multiplier,
                                               in_[i]->length * multiplier));
    }
    return child_data;
  }
  ...
  Result<ArrayDataVector> ChildData(size_t index,
    ...
    for (size_t i = 0; i < in_.size(); ++i) {
      ARROW_ASSIGN_OR_RAISE(child_data[i],
                            in_[i]->child_data[index]->SliceSafe(
...
Result<std::shared_ptr<Array>> Concatenate(ArrayVector&& arrays,
                                           MemoryPool* pool) {
  if (arrays.size() == 0) {
  ...
  std::shared_ptr<arrow::DataType> common_type;
  for (size_t i = 0; i < arrays.size(); ++i) {
    if (common_type == nullptr) {
      common_type = arrays[i]->type();
    }
    if (!arrays[i]->type()->Equals(*common_type)) {
      return Status::Invalid(
          "arrays to be concatenated must be identically typed, but ",
          *arrays[0]->type(), " and ", *arrays[i]->type(),
          " were encountered.");
    }
    data[i] = arrays[i]->data();

    arrays[i].reset();  // release the reference
  }
...
Result<std::shared_ptr<Buffer>> ConcatenateBuffers(
    std::vector<std::shared_ptr<Buffer>>&& buffers, MemoryPool* pool) {
  int64_t out_length = 0;
  for (const auto& buffer : buffers) {
