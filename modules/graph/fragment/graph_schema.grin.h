...
#ifndef MODULES_GRAPH_FRAGMENT_GRAPH_SCHEMA_GRIN_H_
#define MODULES_GRAPH_FRAGMENT_GRAPH_SCHEMA_GRIN_H_
...
class Entry {
  // it seems entry is mostly for schema edition
...
class PropertyGraphSchema {
 public:
 // try to get rid of labelId and propertyId
 // also put get_vertex_id & get_vertex_by_id methods in continous_vid_trait
 // same can be done for labelId and propertyId
  using LabelId = int;
  using PropertyId = int;
  ...
  */
  ...
  //mutable void FromJSON(json const& root);

  std::string ToJSONString() const;
  //mutable void FromJSONString(std::string const& schema);

  //mutable void set_fnum(size_t fnum) { fnum_ = fnum; }
  ...
  // grin vertex label list
  // grin get vertex name
  ...
  //mutable bool Validate(std::string& message);

  const std::map<std::string, int>& GetPropertyNameToIDMapping() const;
  // get from grin and build
  void DumpToFile(std::string const& path);
  // similar
  // mutable void InvalidateVertex(LabelId label_id) { valid_vertices_[label_id] = 0; }

  // mutable void InvalidateEdge(LabelId label_id) { valid_edges_[label_id] = 0; }

  size_t vertex_label_num() const {
    // grin get_vertex_label_list_size
    return std::accumulate(valid_vertices_.begin(), valid_vertices_.end(), 0);
  }

  size_t edge_label_num() const {
    // grin get_edge_label_list_size
...
/* transformation is currently out of the scope of grin
...
#endif  // MODULES_GRAPH_FRAGMENT_GRAPH_SCHEMA_GRIN_H_
