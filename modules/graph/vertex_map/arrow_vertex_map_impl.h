...
ObjectID ArrowVertexMap<OID_T, VID_T>::AddNewVertexLabels(
  ...
  return addNewVertexLabels(client, arrays);
}
...
ObjectID ArrowVertexMap<arrow_string_view, VID_T>::addNewVertexLabels(
  ...
  auto fn = [this, &client, &oid_arrays, &vy_oid_arrays](
                const label_id_t label, const fid_t fid) -> Status {
    vineyard::HashmapBuilder<oid_t, vid_t> builder(client);
  ...
  for (int i = 0; i < thread_num; ++i) {
    threads[i] = std::thread([&]() {
      while (true) {
        ...
        int64_t vnum = array->length();
        o2g_[cur_fid][cur_label].reserve(static_cast<size_t>(vnum));
        for (int64_t k = 0; k < vnum; ++k) {
          o2g_[cur_fid][cur_label].emplace(array->GetView(k), cur_gid);
          ++cur_gid;
        }
        // shrink the size of hashmap
        o2g_[cur_fid][cur_label].shrink_to_fit();
      }
    });
  }
...
void ArrowVertexMapBuilder<arrow_string_view, VID_T>::set_oid_array(
    fid_t fid, label_id_t label,
    const typename std::shared_ptr<InternalType<oid_t>::vineyard_array_type>&
        array) {
  oid_arrays_[fid][label] = *array;
}
...
BasicArrowVertexMapBuilder<OID_T, VID_T>::BasicArrowVertexMapBuilder(
  ...
  for (label_id_t label = 0; label < label_num; ++label) {
    ...
    for (fid_t fid = 0; fid < fnum; ++fid) {
      oid_arrays_[label][fid].push_back(oid_arrays[label][fid]);
    }
  }
  id_parser_.Init(fnum_, label_num_);
}
...
vineyard::Status BasicArrowVertexMapBuilder<arrow_string_view, VID_T>::Build(
  ...
  auto fn = [&](const label_id_t label, const fid_t fid) -> Status {
    typename InternalType<oid_t>::vineyard_builder_type array_builder(
        client, std::move(oid_arrays_[label][fid]));
    auto varray = std::dynamic_pointer_cast<vineyard_oid_array_t>(
        array_builder.Seal(client));
    this->set_oid_array(fid, label, varray);
    return Status::OK();
  };
