...
boost::leaf::result<std::shared_ptr<arrow::Table>> ShufflePropertyEdgeTable(
  ...
  grape::BlockingQueue<std::pair<fid_t, grape::InArchive>> msg_out;
  grape::BlockingQueue<grape::OutArchive> msg_in;

  msg_out.SetProducerNum(serialize_thread_num);
  msg_in.SetProducerNum(1);

  int64_t record_batches_to_send = static_cast<int64_t>(record_batch_num);
  int64_t total_record_batches;
  MPI_Allreduce(&record_batch_num, &total_record_batches, 1, MPI_INT64_T,
  ...
  for (int i = 0; i != serialize_thread_num; ++i) {
    serialize_threads[i] = std::thread([&]() {
      while (true) {
        ...
        if (got_batch >= record_batch_num) {
          break;
        }
        auto cur_rb = record_batches[got_batch];
  ...
  for (int i = 0; i != deserialize_thread_num; ++i) {
    deserialize_threads[i] = std::thread([&]() {
      ...
      while (msg_in.Get(arc)) {
        ...
        DeserializeSelectedRows(arc, table_in->schema(), batches_in[got_batch]);
      }
    });
  }
  ...
  for (size_t rb_i = 0; rb_i != record_batch_num; ++rb_i) {
    std::shared_ptr<arrow::RecordBatch> rb;
    SelectRows(record_batches[rb_i], offset_lists[rb_i][comm_spec.fid()], rb);
    divided_record_batches[comm_spec.fid()].emplace_back(std::move(rb));
  }
  for (auto& rb : batches_in) {
    divided_record_batches[comm_spec.fid()].emplace_back(std::move(rb));
  }
  ...
  std::thread send_thread([&]() {
    for (int i = 1; i != worker_num; ++i) {
      ...
      fid_t dst_fid = comm_spec.WorkerToFrag(dst_worker_id);
      grape::InArchive arc;
      arc << record_batch_num;
      for (size_t rb_i = 0; rb_i != record_batch_num; ++rb_i) {
        SerializeSelectedRows(arc, record_batches[rb_i],
  ...
  std::thread recv_thread([&]() {
    for (int i = 1; i != worker_num; ++i) {
      ...
      for (size_t rb_i = 0; rb_i != rb_num; ++rb_i) {
        std::shared_ptr<arrow::RecordBatch> rb;
        DeserializeSelectedRows(arc, table_in->schema(), rb);
        divided_record_batches[comm_spec.fid()].emplace_back(std::move(rb));
      }
    }
