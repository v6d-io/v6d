...
#include "graph/utils/thread_group.h"
  ...
  inline Status RecvArrowBuffer(std::shared_ptr<arrow::Buffer> & buffer,
    ...
    int64_t size;
    MPI_Recv(&size, 1, MPI_INT64_T, src_worker_id, tag, comm,
             MPI_STATUS_IGNORE);
  ...
  inline Status recv_numeric_array<std::string>(
    ...
    array =
        std::dynamic_pointer_cast<arrow::StringArray>(batches[0]->column(0));
    return Status::OK();
  }
  ...
  Status FragmentAllGatherArray(
    ...
    auto recv_procedure = [&]() {
      ...
      while (src_worker_id != worker_id) {
        ...
        recv_numeric_array<T>(data_out[src_fid], src_worker_id,
                              comm_spec.comm());
  ...
  Status ShufflePropertyVertexTable(
    ...
    auto recv_procedure = [&]() -> Status {
      ...
      while (src_worker_id != worker_id) {
        std::shared_ptr<arrow::Buffer> buffer;
        RETURN_ON_ERROR(
            RecvArrowBuffer(buffer, src_worker_id, comm_spec.comm()));
        std::vector<std::shared_ptr<arrow::RecordBatch>> batches;
        RETURN_ON_ERROR(DeserializeRecordBatches(buffer, &batches));
