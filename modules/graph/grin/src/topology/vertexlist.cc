...
GRIN_VERTEX_LIST grin_get_vertex_list(GRIN_GRAPH g) {
    ...
    for (auto vtype = 0; vtype < _g->vertex_label_num(); ++vtype) {
        vl->push_back(_GRIN_TYPED_VERTICES_T(vtype, _g->Vertices(vtype)));
    }
    return vl;
}
...
GRIN_VERTEX_LIST grin_get_vertex_list_by_type(GRIN_GRAPH g, GRIN_VERTEX_TYPE vtype) {
    ...
    vl->push_back(_GRIN_TYPED_VERTICES_T(*_vtype, _g->Vertices(*_vtype)));
    return vl;
}
#endif
...
bool grin_insert_vertex_to_list(GRIN_GRAPH g, GRIN_VERTEX_LIST vl, GRIN_VERTEX v) {
    ...
    _vl->push_back(_GRIN_TYPED_VERTICES_T(vtype, GRIN_GRAPH_T::vertex_range_t(_v->GetValue(), _v->GetValue())));
    return true;
}
...
GRIN_VERTEX grin_get_vertex_from_list(GRIN_GRAPH g, GRIN_VERTEX_LIST vl, size_t idx) {
    ...
    for (auto &tv : *_vl) {
        result += tv.second.size();
        if (idx < result) {
            auto _idx = idx - (result - tv.second.size());
            auto v = new GRIN_VERTEX_T(tv.second.begin_value() + _idx);
            return v;
        }
    }
