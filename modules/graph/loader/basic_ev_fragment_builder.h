...
#ifndef MODULES_GRAPH_LOADER_BASIC_EV_FRAGMENT_BUILDER_H_
#define MODULES_GRAPH_LOADER_BASIC_EV_FRAGMENT_BUILDER_H_
...
class BasicEVFragmentBuilder {
  ...
  using range_table_t = std::pair<std::vector<range_t>, std::shared_ptr<arrow::Table>>;

 public:
  explicit BasicEVFragmentBuilder(Client& client,
                                 const grape::CommSpec& comm_spec,
                                 const std::vector<PARTITIONER_T>& partitioners,
                                 const std::vector<std::vector<std::vector<oid_t>>>& oid_list,
  ...
  boost::leaf::result<void> ConstructVertices(
    ...
    for (label_id_t v_label = 0; v_label < vertex_label_num_; ++v_label) {
      ...
      output_vertex_tables_[v_label] = vertex_table->ReplaceSchemaMetadata(metadata);
    }
  ...
  boost::leaf::result<void> AddEdgeTable(
      ...
      const std::string& edge_label, std::shared_ptr<arrow::Table> adj_list_table,
      std::shared_ptr<arrow::Array> offset_array, std::shared_ptr<arrow::Table> property_table) {
    ...
    input_edge_tables_[edge_label] = std::make_pair(
        std::make_pair(src_label_id, dst_label_id),
        std::make_tuple(adj_list_table, offset_array, property_table));
  ...
  boost::leaf::result<void> ConstructEdges(int label_offset = 0,
    ...
    for (label_id_t e_label = 0; e_label < edge_label_num_; ++e_label) {
      ...
      output_edge_tables_[e_label] = std::make_tuple(table,
          std::get<1>(ordered_edge_tables_[e_label].second), std::get<2>(ordered_edge_tables_[e_label].second)->ReplaceSchemaMetadata(metadata));
    }
    ordered_edge_tables_.clear();
    return {};
  }
 ...
 private:
  ...
  parseOidChunkedArray(label_id_t label_id,
    ...
    for (size_t chunk_i = 0; chunk_i != chunk_num; ++chunk_i) {
      ...
      for (size_t i = 0; i != size; ++i) {
        ...
        fid_t fid;
        try {
          fid = partitioners_[label_id].GetPartitionId(oid_t(oid));
        }
        catch (...) {
          LOG(INFO) << "Catch error: " << oid;
        }
  ...
  boost::leaf::result<void> initSchema(PropertyGraphSchema& schema) {
    ...
    for (label_id_t e_label = 0; e_label != edge_label_num_; ++e_label) {
      ...
      auto table = std::get<2>(output_edge_tables_[e_label]);
  ...
  const std::vector<PARTITIONER_T>& partitioners_;
  const std::vector<std::vector<std::vector<oid_t>>>& oid_lists_;
  ...
  std::map<std::string, std::pair<std::pair<label_id_t, label_id_t>,
          std::tuple<std::shared_ptr<arrow::Table>, std::shared_ptr<arrow::Array>, std::shared_ptr<arrow::Table>>>>
      input_edge_tables_;
  ...
  std::vector<std::tuple<std::shared_ptr<arrow::Table>, std::shared_ptr<arrow::Array>, std::shared_ptr<arrow::Table>>> output_edge_tables_;
