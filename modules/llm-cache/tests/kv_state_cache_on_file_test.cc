...
constexpr int STOREDTOKENS = 4;
const std::string PATH = "/tmp/vineyard/llm";

std::shared_ptr<KVStateCacheManager> manager;

void init() {
  VINEYARD_CHECK_OK(KVStateCacheManager::Make(manager, CHUNKSIZE, SPLITNUMBER, LAYER,
                                              TENSORBYTES, STOREDTOKENS, PATH));
}
...
std::map<int, std::pair<LLMKV, LLMKV>> generate_kv_state(int token) {
  ...
  for (int currentLayer = 0; currentLayer < LAYER; currentLayer++) {
    ...
    for (int i = 0; i < TENSORBYTES; i++) {
      ((char*)key_state.data)[i] = (char)token;
      ((char*)value_state.data)[i] = (char)token;
    }
    kv_state.insert(
