...
import io.v6d.arrow.Int32Builder;
...
@FFITypeAlias("vineyard::ConvertToArrowType<int>::BuilderType")
@FFIGen
@CXXHead("basic/ds/arrow_utils.h")
public interface BuilderTypeInt extends CXXPointer {
    @FFIExpr("{0}")
    Int32Builder get();
    ...
    static BuilderTypeInt cast(final long __foreign_address) {
        try {
            Class<BuilderTypeInt> clz = (Class<BuilderTypeInt>) FFITypeFactory.getType(FFITypeFactory.getFFITypeName(BuilderTypeInt.class, true));
    ...
    static BuilderTypeInt cast(final FFIPointer __foreign_pointer) {
        return BuilderTypeInt.cast(__foreign_pointer.getAddress());
    }

    static Factory getFactory() {
        return FFITypeFactory.getFactory(FFITypeFactory.getFFITypeName(BuilderTypeInt.class, true));
    }

    static BuilderTypeInt create(
            @CXXReference @FFITypeAlias("const std::shared_ptr<arrow::DataType>") shared_ptr<DataType> type,
            MemoryPool pool) {
        return BuilderTypeInt.getFactory().create(type, pool);
    }

    @FFIFactory
    @CXXHead("basic/ds/arrow_utils.h")
    interface Factory {
        BuilderTypeInt create(
                @CXXReference @FFITypeAlias("const std::shared_ptr<arrow::DataType>") shared_ptr<DataType> type,
                MemoryPool pool);
    }
}
