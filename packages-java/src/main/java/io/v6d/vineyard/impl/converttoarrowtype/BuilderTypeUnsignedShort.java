...
import io.v6d.arrow.UInt16Builder;
...
@FFITypeAlias("vineyard::ConvertToArrowType<unsigned short>::BuilderType")
@FFIGen
@CXXHead("basic/ds/arrow_utils.h")
public interface BuilderTypeUnsignedShort extends CXXPointer {
    @FFIExpr("{0}")
    UInt16Builder get();
    ...
    static BuilderTypeUnsignedShort cast(final long __foreign_address) {
        try {
            Class<BuilderTypeUnsignedShort> clz = (Class<BuilderTypeUnsignedShort>) FFITypeFactory.getType(FFITypeFactory.getFFITypeName(BuilderTypeUnsignedShort.class, true));
    ...
    static BuilderTypeUnsignedShort cast(final FFIPointer __foreign_pointer) {
        return BuilderTypeUnsignedShort.cast(__foreign_pointer.getAddress());
    }

    static Factory getFactory() {
        return FFITypeFactory.getFactory(FFITypeFactory.getFFITypeName(BuilderTypeUnsignedShort.class, true));
    }

    static BuilderTypeUnsignedShort create(
            @CXXReference @FFITypeAlias("const std::shared_ptr<arrow::DataType>") shared_ptr<DataType> type,
            MemoryPool pool) {
        return BuilderTypeUnsignedShort.getFactory().create(type, pool);
    }

    @FFIFactory
    @CXXHead("basic/ds/arrow_utils.h")
    interface Factory {
        BuilderTypeUnsignedShort create(
                @CXXReference @FFITypeAlias("const std::shared_ptr<arrow::DataType>") shared_ptr<DataType> type,
                MemoryPool pool);
    }
}
