...
import io.v6d.arrow.UInt8Builder;
...
@FFITypeAlias("vineyard::ConvertToArrowType<unsigned char>::BuilderType")
@FFIGen
@CXXHead("basic/ds/arrow_utils.h")
public interface BuilderTypeUnsignedChar extends CXXPointer {
    @FFIExpr("{0}")
    UInt8Builder get();
    ...
    static BuilderTypeUnsignedChar cast(final long __foreign_address) {
        try {
            Class<BuilderTypeUnsignedChar> clz = (Class<BuilderTypeUnsignedChar>) FFITypeFactory.getType(FFITypeFactory.getFFITypeName(BuilderTypeUnsignedChar.class, true));
    ...
    static BuilderTypeUnsignedChar cast(final FFIPointer __foreign_pointer) {
        return BuilderTypeUnsignedChar.cast(__foreign_pointer.getAddress());
    }

    static Factory getFactory() {
        return FFITypeFactory.getFactory(FFITypeFactory.getFFITypeName(BuilderTypeUnsignedChar.class, true));
    }

    static BuilderTypeUnsignedChar create(
            @CXXReference @FFITypeAlias("const std::shared_ptr<arrow::DataType>") shared_ptr<DataType> type,
            MemoryPool pool) {
        return BuilderTypeUnsignedChar.getFactory().create(type, pool);
    }

    @FFIFactory
    @CXXHead("basic/ds/arrow_utils.h")
    interface Factory {
        BuilderTypeUnsignedChar create(
                @CXXReference @FFITypeAlias("const std::shared_ptr<arrow::DataType>") shared_ptr<DataType> type,
                MemoryPool pool);
    }
}
