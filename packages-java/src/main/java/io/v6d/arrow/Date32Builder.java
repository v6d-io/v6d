...
@FFITypeAlias("arrow::Date32Builder")
@FFIGen
@CXXHead(
        system = "arrow/type_fwd.h"
)
public interface Date32Builder extends CXXPointer {
    @FFIExpr("{0}")
    NumericBuilder<Date32Type> get();
    ...
    static Date32Builder cast(final long __foreign_address) {
        try {
            Class<Date32Builder> clz = (Class<Date32Builder>) FFITypeFactory.getType(FFITypeFactory.getFFITypeName(Date32Builder.class, true));
    ...
    static Date32Builder cast(final FFIPointer __foreign_pointer) {
        return Date32Builder.cast(__foreign_pointer.getAddress());
    }

    static Factory getFactory() {
        return FFITypeFactory.getFactory(FFITypeFactory.getFFITypeName(Date32Builder.class, true));
    }

    static Date32Builder create(
            @CXXReference @FFITypeAlias("const std::shared_ptr<arrow::DataType>") shared_ptr<DataType> type,
            MemoryPool pool) {
        return Date32Builder.getFactory().create(type, pool);
    }
