# The file cmake/DetermineImplicitIncludes.cmake, a workaround for bugs
# in older versions of CMake, is backported from project
# https://github.com/Kitware/CMake
#
#   https://github.com/Kitware/CMake/blob/master/Modules/CMakeParseImplicitIncludeInfo.cmake
#
...
function(cmake_parse_implicit_include_info text lang dir_var log_var state_var)
  ...
  foreach(line IN LISTS output_lines)
    if(state STREQUAL start)
      ...
      if(NOT (rv GREATER -1))
        string(FIND "${line}" "#include \"...\" 搜索从这里开始：" rv)
      endif()
      if(rv GREATER -1)
        set(state loading)
    ...
    elseif(state STREQUAL loading)
      ...
      if(NOT (rv GREATER -1))
        string(FIND "${line}" "#include \"...\" 搜索列表结束。" rv)
      endif()
      if(rv GREATER -1)
        set(state done)
...
# default the default implicit include directories, as a workaround for bugs
# in older CMake versions (available since 3.14).
#
#   https://gitlab.kitware.com/cmake/cmake/-/issues/16291
#
# Part of this file is taken from:
#
#   https://github.com/Kitware/CMake/blob/master/Modules/CMakeParseImplicitIncludeInfo.cmake
#
function(determine_implicit_includes lang result_var)
    ...
    cmake_parse_implicit_include_info("${OUTPUT}" "${lang}" implicit_incdirs log rv)
    set(${result_var} "${implicit_incdirs}" PARENT_SCOPE)
endfunction()
